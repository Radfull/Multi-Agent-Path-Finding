...
class LazyAStarFollowingConflict():
    def __init__(
        self,
    ...
    def _search(self) -> list[State]:
        ...
        # Lazy A*: в кучу добавляем только g_score, h_score будет вычисляться "лениво" при извлечении
        # Для начального узла вычисляем h_score заранее
        h_score_initial = self._admissible_heuristic(initial_state)
        f_score_initial = g_score[initial_state] + self.weight * h_score_initial
        heapq.heappush(
            heap, (f_score_initial, h_score_initial, next(index), initial_state)
        )
        ...
        while open_set and (self.a_star_max_iter == -1 or self.iter < self.a_star_max_iter):
            ...
            # Извлекаем узел из кучи
            heap_item = heapq.heappop(heap)
            f_score_stored, h_score_stored, _, current = heap_item

            # Lazy evaluation: если узел уже в closed_set, пропускаем (он был обновлен и уже в куче)
            if current in closed_set:
                continue

            # Lazy evaluation: перевычисляем f_score при извлечении
            # Это ключевое отличие lazy A* - эвристика вычисляется именно при извлечении, а не при добавлении
            g_current = g_score.get(current, float("inf"))
            
            # Вычисляем эвристику "лениво" - только сейчас, при извлечении узла
            h_current = self._admissible_heuristic(current)
            f_current = g_current + self.weight * h_current

            # Если вычисленный f_score больше хранимого (с учетом погрешности), 
            # значит g_score узла был обновлен после добавления в кучу
            # Добавляем обратно в кучу с актуальным значением
            if abs(f_current - f_score_stored) > 1e-9 and f_current > f_score_stored:
                heapq.heappush(heap, (f_current, h_current, next(index), current))
                continue
            ...
            for neighbor in neighbor_list:
                ...
                # Lazy A*: вычисляем эвристику при добавлении в кучу
                # Но при извлечении она будет пересчитана для проверки актуальности
                h_score = self._admissible_heuristic(neighbor)
                f_score = tentative_g_score + self.weight * h_score + dst_add
                
                heapq.heappush(heap, (f_score, h_score, next(index), neighbor))
        return False
