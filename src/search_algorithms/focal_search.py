...
class FocalSearchFollowingConflict():
    ...
    def _secondary_heuristic(self, state: State) -> float:
        """
        Improved secondary heuristic for focal search.
        Consider both distance to goal and potential conflicts.
        """
        goal = self.agent["goal"]
        ...
        return primary_h + time_penalty + obstacle_penalty + conflict_penalty
    ...
    def _search(self) -> list[State]:
        ...
        while open_set and (self.focal_max_iter == -1 or self.iter < self.focal_max_iter):
            ...
            if open_heap:
                ...
                if not focal_set or current_min_f > min_f_score * 1.1:
            ...
            if not focal_set:
                ...
                current = heapq.heappop(focal_set)[2]
                open_heap = [(f, h, idx, node) for (f, h, idx, node) in open_heap if node != current]
            ...
            for neighbor in neighbor_list:
                ...
                if neighbor not in open_set:
                    ...
                    heapq.heappush(
                        open_heap, 
                        (f_score[neighbor], h_score, next(index), neighbor)
                    )
                    if f_score[neighbor] <= self.w * min_f_score:
                        heapq.heappush(
                            focal_set,
                    ...
                    g_score[neighbor] = tentative_g_score
                    h_score = self._admissible_heuristic(neighbor)
                    new_f_score = g_score[neighbor] + h_score + dst_add
                    
                    open_heap = [(f, h, idx, node) for (f, h, idx, node) in open_heap if node != neighbor]
                    heapq.heappush(
                        open_heap, 
                        (new_f_score, h_score, next(index), neighbor)
                    )
                    
                    focal_set = [(sh, idx, node) for (sh, idx, node) in focal_set if node != neighbor]
